...
{			
	// Create battle row
	$sts_battle = new sql_table_struct_Battle();
	$sts_battle->status = 0;
	$sts_battle->current_step = 0;
	$sts_battle->created_time = time();	
	$sql->insertRowExt("battles", $sts_battle);	
	...
	$sts_char_battle->is_owner = 1; // 0 - is not owner, 1 - is owner
	...
	$starfight->SendAnswer( 0, 0, "Battle lobby successful created!" );
	
	// Запускаю ожидание начала боя
	$starfight->Timer("php/background_processes/battle/lobby.php", 40, "battle_id=" . $battle_id);

}
else
{
	$starfight->SendAnswer( 0, 1, "Вы уже в бою!" );
}
